// This is mainly just for the index.html page with all the wierd
// js gimicks. Just for fun.
var DEBUG = true;
if(!DEBUG){
...
function rotatePhotos(myJson){
    ...
    var newval = setInterval(function(){
        if(i<lines.length){
            ...
            let picData = newJson[lines[i]];
            let picDataArray = [];
            for(let key of Object.keys(picData)){
                picDataArray.push(key + ": " + picData[key]);
            }
            charWrite(picDataArray,50,'#centerBottomDiv');
            i++;
        }else{
            i=0;
        }
...
function onesZeros(l=0){
    var len = [];
    var buh = setInterval(function(){

        for(var i =0; i < 250;i++){
            var rand = myRandomGen(1,10);
            var num = myRandomGen(0,1);
            if(rand == 10){
                len.push('<strong>' + num + '</strong>');
            }else{
                len.push(num);
            }
            len.push(" ");
